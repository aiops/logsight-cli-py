...
def upload(ctx, file, tag, app_id):
    ...
    python -m logsight_cli.logsight_cli log upload <file> --tag v1 --app_id <applicationId>
    """
...
def ls(ctx, app_id):
    ...
    python -m logsight_cli.logsight_cli log tag ls --app_id <applicationId>
    """
    auth = ctx.obj['AUTHENTICATION']
    a = app_id or ctx.obj['APP_ID']

    try:

        cmp_mng = LogsightCompare(auth.user_id, auth.token)
